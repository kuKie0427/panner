...
def parse_arguments():
    parser = argparse.ArgumentParser(description="Runs an agent powered by the given model on panner benchmark.")
    parser.add_argument(
        "--date",
        type=str,
        default=None,
    ...
    parser.add_argument(
        "--eval-dataset",
        type=str,
        default="panner/benchmark-v1",
    )
    # The eval dataset is gated, so you must first visit its page to request access: https://huggingface.co/datasets/panner-benchmark/benchmark-v1
    parser.add_argument(
        "--model-type",
        type=str,
