...
This example demonstrates how to use a `CodeAgent` from the `panner` library in an asynchronous Starlette web application.
...
- **CodeAgent**: An agent from the `panner` library that can be used to solve tasks programmatically.
...
1. **Install dependencies**:
   ```bash
   pip install panner starlette anyio uvicorn
   ```
