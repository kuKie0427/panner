...
from panner.default_tools import FinalAnswerTool, WikipediaSearchTool
from panner.local_python_executor import CodeOutput
from panner.monitoring import AgentLogger, LogLevel
from panner.remote_executors import (
    BlaxelExecutor,
    DockerExecutor,
    E2BExecutor,
    ModalExecutor,
...
from panner.serialization import SerializationError
from panner.utils import AgentError
...
class TestModalExecutorUnit:
    @patch("panner.remote_executors._websocket_run_code_raise_errors")
    @patch("requests.post")
    @patch("requests.get")
    @patch("websocket.create_connection")
...
class TestBlaxelExecutorUnit:
    ...
    @patch("panner.remote_executors._create_kernel_http")
    ...
    def test_blaxel_executor_instantiation_with_blaxel_sdk(
        ...
        assert executor.sandbox_name.startswith("panner-executor-")
        assert executor.image == "blaxel/jupyter-notebook"
        assert executor.memory == 4096
        assert executor.region is None

    @patch("panner.remote_executors.BlaxelExecutor.install_packages")
    @patch("panner.remote_executors._create_kernel_http")
    @patch("blaxel.core.SandboxInstance")
    @patch("blaxel.core.settings")
