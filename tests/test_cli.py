...
from panner.cli import load_model
from panner.local_python_executor import CodeOutput, LocalPythonExecutor
from panner.models import InferenceClientModel, LiteLLMModel, OpenAIModel, TransformersModel
...
def test_cli_main(capsys):
    with patch("panner.cli.load_model") as mock_load_model:
        mock_load_model.return_value = "mock_model"
        with patch("panner.cli.CodeAgent") as mock_code_agent:
            from panner.cli import run_panner

            run_panner("test_prompt", [], "InferenceClientModel", "test_model_id", provider="hf-inference")
    # load_model
...
def test_vision_web_browser_main():
    with patch("panner.vision_web_browser.helium"):
        with patch("panner.vision_web_browser.load_model") as mock_load_model:
            mock_load_model.return_value = "mock_model"
            with patch("panner.vision_web_browser.CodeAgent") as mock_code_agent:
                from panner.vision_web_browser import helium_instructions, run_webagent
