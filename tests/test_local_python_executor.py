...
from panner.default_tools import BASE_PYTHON_TOOLS, FinalAnswerTool
from panner.local_python_executor import (
    DANGEROUS_FUNCTIONS,
    DANGEROUS_MODULES,
...
recur_fibo(6)"""
    ...
    def test_max_operations(self):
        ...
        with patch("panner.local_python_executor.MAX_OPERATIONS", 100):
            with pytest.raises(InterpreterError) as exception_info:
...
"""
    ...
    def test_while(self):
        ...
        # test infinite loop
        code = "i = 0\nwhile i < 3:\n    i -= 1\ni"
        with patch("panner.local_python_executor.MAX_WHILE_ITERATIONS", 100):
...
class TestLocalPythonExecutorSecurity:
    ...
    def test_vulnerability_via_dunder_call(
        ...
        import panner.local_python_executor

        monkeypatch.setattr(
            "panner.local_python_executor.ALLOWED_DUNDER_METHODS",
            panner.local_python_executor.ALLOWED_DUNDER_METHODS + additional_allowed_dunder_methods,
        )
        executor = LocalPythonExecutor([])
        executor.send_tools({})
        expectation = (
