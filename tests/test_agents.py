...
from panner import EMPTY_PROMPT_TEMPLATES
from panner.agent_types import AgentImage, AgentText
from panner.agents import (
    AgentError,
    AgentMaxStepsError,
    AgentToolCallError,
    CodeAgent,
...
from panner.default_tools import DuckDuckGoSearchTool, FinalAnswerTool, PythonInterpreterTool, VisitWebpageTool
from panner.memory import (
    ActionStep,
    CallbackRegistry,
    FinalAnswerStep,
    MemoryStep,
...
from panner.monitoring import AgentLogger, LogLevel, Timing, TokenUsage
from panner.tools import Tool, tool
from panner.utils import (
...
class TestMultiStepAgent:
    ...
    def test_from_dict(self):
        ...
        with patch.dict("panner.models.MODEL_REGISTRY", {"TransformersModel": mock_model_class}):
    ...
    def test_multiagent_to_dict_from_dict_roundtrip(self):
        ...
        with patch.dict("panner.models.MODEL_REGISTRY", {"MagicMock": mock_model_class}):
...
class TestToolCallingAgent:
    ...
    def test_toolcalling_agent_stream_logs_multiple_tool_calls_observations(self, mock_openai_client, test_tool):
        ...
        from panner import OpenAIModel
...
print("Ok, calculation done!")""")
    ...
    def test_from_folder(self, agent_dict_version, get_agent_dict):
        ...
        with (
            patch("panner.agents.Path") as mock_path,
            patch.dict("panner.models.MODEL_REGISTRY", {"InferenceClientModel": mock_model_class}),
        ):
            import json
    ...
    def test_from_dict(self):
        ...
        with patch.dict("panner.models.MODEL_REGISTRY", {"InferenceClientModel": mock_model_class}):
