...
from panner.agent_types import AgentAudio, AgentImage, AgentText
from panner.agents import MultiStepAgent, PlanningStep
from panner.memory import ActionStep, FinalAnswerStep
from panner.models import ChatMessageStreamDelta, MessageRole, agglomerate_stream_deltas
from panner.utils import _is_package_available
...
def pull_messages_from_step(step_log: ActionStep | PlanningStep | FinalAnswerStep, skip_model_outputs: bool = False):
    ...
    if not _is_package_available("gradio"):
        raise ModuleNotFoundError(
            "Please install 'gradio' extra to use the GradioUI: `pip install 'panner[gradio]'`"
        )
...
class GradioUI:
    ...
    This class requires the `gradio` extra to be installed: `pip install 'panner[gradio]'`.
    ...
    Example:
        ```python
        from panner import CodeAgent, GradioUI, InferenceClientModel
    ...
    def create_app(self):
        ...
        chatbot = gr.Chatbot(
            ...
            avatar_images=(
                None,
                "https://huggingface.co/datasets/huggingface/documentation-images/resolve/main/panner/mascot_smol.png",
            ),
            latex_delimiters=[
