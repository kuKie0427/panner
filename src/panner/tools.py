...
class Tool(BaseTool):
    ...
    def to_dict(self) -> dict:
        ...
        requirements = {el for el in get_imports(tool_code) if el not in sys.stdlib_module_names} | {"panner"}
    ...
    def _initialize_hub_repo(repo_id: str, token: bool | str | None, private: bool | None) -> str:
        ...
        metadata_update(repo_url.repo_id, {"tags": ["panner", "tool"]}, repo_type="space", token=token)
    ...
    def _get_gradio_app_code(self, tool_module_name: str = "tool") -> str:
        ...
        return textwrap.dedent(
            f"""\
            from panner import launch_gradio_demo
...
class ToolCollection:
    ...
    def from_hub(
        ...
        Example:
        ```py
        >>> from panner import ToolCollection, CodeAgent
    ...
    def from_mcp(
        ...
        ```py
        >>> import os
        >>> from panner import ToolCollection, CodeAgent, InferenceClientModel
        ...
        if structured_output is None:
            warnings.warn(
                ...
                "See documentation at https://huggingface.co/docs/panner/tutorials/tools#structured-output-and-output-schema-support for more details.",
                FutureWarning,
                stacklevel=2,
            )
        ...
        try:
            ...
            from mcpadapt.panner_adapter import SmolAgentsAdapter
        except ImportError:
            raise ImportError(
                """Please install 'mcp' extra to use ToolCollection.from_mcp: `pip install 'panner[mcp]'`."""
            )
        ...
        **hub_kwargs,
        ...
        if not _is_package_available("accelerate") or not _is_package_available("torch"):
            raise ModuleNotFoundError(
                "Please install 'transformers' extra to use a PipelineTool: `pip install 'panner[transformers]'`"
            )
...
def get_tools_definition_code(tools: dict[str, Tool]) -> str:
    ...
    for tool in tools.values():
        ...
        tool_code = tool_code.replace("from panner.tools import Tool", "")
