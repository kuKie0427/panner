...
class RemotePythonExecutor(PythonExecutor):
    ...
    def send_tools(self, tools: dict[str, Tool]):
        ...
        packages_to_install = {
            ...
            if pkg not in self.installed_packages + ["panner"]
        }
        ...
        **kwargs,
        ...
        try:
            ...
            raise ModuleNotFoundError(
                """Please install 'e2b' extra to use E2BExecutor: `pip install 'panner[e2b]'`"""
            )
...
class DockerExecutor(RemotePythonExecutor):
    ...
    def __init__(
        ...
        try:
            ...
            raise ModuleNotFoundError(
                "Please install 'docker' extra to use DockerExecutor: `pip install 'panner[docker]'`"
            )
        self.host = host
        self.port = port
...
class ModalExecutor(RemotePythonExecutor):
    ...
    Args:
        ...
        allow_pickle (`bool`, default `False`): Whether to allow pickle serialization for objects that cannot be safely serialized to JSON.
            ...
            **Security Warning:** Pickle deserialization can execute arbitrary code. Only set `allow_pickle=True`
            if you fully trust the execution environment and need backward compatibility with custom types.
        app_name (`str`, default `"panner-executor"`): App name.
    ...
    def __init__(
        ...
        try:
            ...
            raise ModuleNotFoundError(
                """Please install 'modal' extra to use ModalExecutor: `pip install 'panner[modal]'`"""
            )
...
class BlaxelExecutor(RemotePythonExecutor):
    ...
    Args:
        ...
        allow_pickle (`bool`, default `False`): Whether to allow pickle serialization for objects that cannot be safely serialized to JSON.
            ...
            **Security Warning:** Pickle deserialization can execute arbitrary code. Only set `allow_pickle=True`
            if you fully trust the execution environment and need backward compatibility with custom types.
        sandbox_name (`str`, *optional*): Name for the sandbox. Defaults to "panner-executor".
    ...
    def __init__(
        ...
        try:
            ...
            raise ModuleNotFoundError(
                "Please install 'blaxel' extra to use BlaxelExecutor: `pip install 'panner[blaxel]'`"
            )

        self.sandbox_name = sandbox_name or f"panner-executor-{uuid.uuid4().hex[:8]}"
        self.image = image
        self.memory = memory
        self.region = region
        self.port = 8888
